...
briefRouter.post('/', async (req: Request, res: Response) => {
  const raw = req.body ?? {}
  ...
  const update: Record<string, unknown> = { appointment_id: appointmentId }

  for (const campo of CAMPOS_TEXTO) {
    const val = (respuestasRaw as Record<string, unknown>)[campo]
    if (typeof val === 'string') update[campo] = stripHtml(val).slice(0, 4000)
  }

  const servicios = (respuestasRaw as Record<string, unknown>).servicios_solicitados
  if (Array.isArray(servicios)) {
    update.servicios_solicitados = servicios.filter(
      (v): v is string => typeof v === 'string' && SERVICIOS_VALIDOS.includes(v)
    )
  }
