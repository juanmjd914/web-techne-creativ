...
function checkSession(token: string | undefined): boolean {
  return !!token && sessions.has(token)
}

function tokenFrom(req: Request): string | undefined {
  return (req.headers['x-admin-token'] as string) ?? (req.query.token as string)
}
...
adminRouter.patch('/appointments/:id', async (req: Request, res: Response) => {
  if (!checkSession(tokenFrom(req))) { res.status(401).json({ error: 'No autorizado' }); return }
  const { status } = req.body

  const { data: appt, error: fetchErr } = await supabase
    .from('appointments').select('*').eq('id', req.params.id).single()
  if (fetchErr || !appt) { res.status(404).json({ error: 'Cita no encontrada' }); return }
...
adminRouter.post('/appointments/:id/calendar', async (req: Request, res: Response) => {
  if (!checkSession(tokenFrom(req))) { res.status(401).json({ error: 'No autorizado' }); return }
...
adminRouter.delete('/appointments/:id', async (req: Request, res: Response) => {
  if (!checkSession(tokenFrom(req))) { res.status(401).json({ error: 'No autorizado' }); return }
...
adminRouter.patch('/messages/:id', async (req: Request, res: Response) => {
  if (!checkSession(tokenFrom(req))) { res.status(401).json({ error: 'No autorizado' }); return }
...
adminRouter.get('/demo-requests', async (req: Request, res: Response) => {
  if (!checkSession(tokenFrom(req))) { res.status(401).json({ error: 'No autorizado' }); return }
  const { data, error } = await supabase
    .from('demo_requests')
    .select('*')
...
adminRouter.patch('/demo-requests/:id', async (req: Request, res: Response) => {
  if (!checkSession(tokenFrom(req))) { res.status(401).json({ error: 'No autorizado' }); return }
  const { status } = req.body
...
adminRouter.delete('/demo-requests/:id', async (req: Request, res: Response) => {
  if (!checkSession(tokenFrom(req))) { res.status(401).json({ error: 'No autorizado' }); return }
